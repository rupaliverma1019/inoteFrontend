...
const getNotes = async() => {
  try {
    // API CALL
    const response = await fetch("https://notebook-r1df.onrender.com/api/notes/fetchallnotes", {
      method: "GET",
      headers: {
...
const addNote = async (title, description, tag) => {
  // API CALL
  const response = await fetch("https://notebook-r1df.onrender.com/api/notes/addnote", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "auth-token": localStorage.getItem('token')
...
const editNote = async(id, title, description, tag)=>{
  // API CALL
  const response = await fetch(`https://notebook-r1df.onrender.com/api/notes/updatenote/${id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      "auth-token": localStorage.getItem('token')
...
// Api Call
const response = await fetch(`https://notebook-r1df.onrender.com/api/notes/deletenote/${id}`, {
  method: "DELETE",
  headers: {
    "Content-Type": "application/json",
    "auth-token": localStorage.getItem('token')
